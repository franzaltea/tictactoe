...
# DEBUG models.Models represents a table in the db that holds data
GAME_STATUS_CHOICES = (
...
class Game(models.Model):
    ...
    def board(self):
        """
        Returns a 2-d list of Move objects, so you can ask for the state
        of a square at position [y][x]
        """
        # DEBUG better to replace this with numpy array
    ...
    def is_users_move(self, user):
        """
        Returns true if it is the user's turn to move
        """
    ...
    def new_move(self):
        ...
        if self.status not in 'FS':
            raise ValueError("Cannot move on finished game")
...
class Move(models.Model):
    # DEBUG this adds columns in addition to primary keys in db
